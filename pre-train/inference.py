...
def griffin_lim(stftm_matrix, shape, max_iter=50):
    y = np.random.random(shape)
    for i in range(max_iter):
        stft_matrix = librosa.core.stft(y, n_fft=2048, win_length=800, hop_length=200)
        stft_matrix = stftm_matrix * stft_matrix / np.abs(stft_matrix)
        y = librosa.core.istft(stft_matrix, win_length=800, hop_length=200)
    return y
...
if __name__ == "__main__":
    ...
    # this below condition is trivial but made more sense in original code
    path_save += '_seen' if test_list == True else '_unseen'
